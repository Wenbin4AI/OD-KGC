...
from src.utils import LLM_Model, build_messages
...
def aggregate_metrics(results: List[Dict[str,Any]]) -> Dict[str,Any]:
    total = len(results)
    if total == 0:
        return {"total":0,"MRR":0.0,"Hit@1":0.0,"Hit@3":0.0,"Hit@10":0.0,"failed_count":0,"parse_failed_count":0}
    return {"total":total,
            ...
            "parse_failed_count":sum(1 for x in results if x.get("status")=="failed_parse_filled_by_default_order")}

# ------------------------
# LLM QA Evaluator
# ------------------------
class LLMQAEvaluator:
    def __init__(self, dataset, rotate_manager, llm: LLM_Model, candidate_builder, top_k:int=10, max_retries:int=2, retry_sleep:float=2.0):
    ...
    def evaluate_one_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # generate auxiliary evidence
        prompt_aux = f"Head: {head_label}, Relation: {relation_label}, Classes: {','.join(head_classes)}, Range: {','.join(relation_range)}. Generate one short evidence sentence for KG completion in JSON."
        aux_output = self.query_llm_raw(prompt_aux)
        aux_evidence = self.parse_auxiliary_evidence(aux_output)

        # append auxiliary evidence
        ...
        prompt_ranking = f"Rank candidates from 0-based indices 0..{len(candidates)-1} for query ({head_label}, {relation_label}, ?). Top {self.top_k}. Evidence:\n{evidence_text}\nCandidates:\n" + "\n".join([f"{i}. {c['label']}" for i,c in enumerate(candidates)]) + "\nOutput JSON with selected_indices."
        ...
        result = {"query_index":record.get("query_index"), "pred_indices_final":pred_indices_final, "ranked_entity_ids":ranked_entity_ids, **metrics, "status":status, "llm_generated_evidence":aux_evidence}
        return result

    def query_llm_raw(self,prompt:str)->str:
        for attempt in range(self.max_retries+1):
            try:
                response = self.llm.openai_client.chat.completions.create(**self.llm.llm_config, messages=[{"role":"system","content":"Answer strictly in JSON."},{"role":"user","content":prompt}])
    ...
    def parse_auxiliary_evidence(raw_output:str)->str:
        ...
        try:
            ...
            if extracted:
                data = json.loads(extracted.group(0))
                return data.get("evidence","").strip()
        except:
            pass
        return ""

# ------------------------
# Main parallel evaluation with checkpoint
# ------------------------
def main():
    ...
    # candidate builder
    from src.utils import CandidateBuilder
    ...
    with ThreadPoolExecutor(max_workers=args.parallel_workers) as executor:
        ...
        for future in tqdm(as_completed(future_to_idx), total=len(future_to_idx), desc="LLM QA Evaluation", ncols=100):
            ...
            try:
                ...
                result = {"query_index":record.get("query_index"),"ranked_entity_ids":[],"MRR":0.0,"Hit@1":0,"Hit@3":0,"Hit@10":0,"status":f"failed_exception:{repr(e)}"}

            results.append(result)

            # checkpoint every save_every queries
