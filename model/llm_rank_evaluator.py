...
class LLMRankEvaluator:
    ...
    def evaluate_one_record(
        ...
        if (
            self.debug_print_prompt
            or self.debug_print_candidates
        ) and not self.has_printed_debug:
            print("\n" + "=" * 100)
            print("[Debug] First query")
            print("=" * 100)
            print(f"Query index: {query_index}")
            print(f"Query: ({head_label}, {relation_label}, ?)")
            print(f"Gold tail: {gold_tail_label} | id={gold_tail_id}")
            print(f"Candidate mode: {self.candidate_mode}")
            print(f"Candidate info:\n{json.dumps(candidate_info, ensure_ascii=False, indent=2)}")

            if self.debug_print_candidates:
                print("\n[Debug] Candidates")
                for cand in candidates:
                    mark = " <== GOLD" if cand.get("is_gold") else ""
                    print(
                        f"{cand['index']}. id={cand['entity_id']} | "
                        f"{cand['label']} | score={cand['rotate_score']:.4f} | "
                        f"classes={cand.get('classes')} {mark}"
                    )
...
def main():
    ...
    for idx, record in enumerate(
        ...
        if idx % args.save_every == 0:
            ...
            print(f"Last candidate info: {json.dumps(last.get('candidate_info'), ensure_ascii=False, indent=2)}")
