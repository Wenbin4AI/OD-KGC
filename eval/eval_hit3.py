...
def extract_selected_indices(llm_output: str):
    cleaned_output = clean_llm_output(llm_output)
...
Your task is to select the TOP 3 most likely tail entities from a candidate list.
...
- You MUST select exactly THREE candidate indices.
...
5. Select the TOP 3 most consistent ones.
...
def predict_for_triple(triple: Tuple[int, int, int],
    ...
    # 拼接包含 key evidence 的 Hit@3 prompt
    ...
    # 解析前三个预测序号
    pred_indices = extract_selected_indices(result)
...
def main():
    ...
    final_hit3 = hit_count / len(results) if results else 0.0
    print(f"\nFinal Hit@3: {final_hit3:.4f}")
